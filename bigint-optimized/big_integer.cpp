...
big_integer operator*(big_integer a, big_integer const &b) {
  ...
  for (size_t i = 0; i < a.value_.size(); ++i) {
    uint32_t carry = 0;
    for (size_t j = 0; j < b.value_.size(); ++j) {
    ...
    res.value_[i + b.value_.size()] = carry;
  }
  res.to_normal_form();
  return res;
}
...
bool big_integer::small_two_degree(big_integer &a) const {
  if (a.size() != 1) {
    return false;
  }
...
big_integer operator/(big_integer a, big_integer b) {
  size_t zeroes = 0;
  while (a.value_[zeroes] == 0 && b.value_[zeroes] == 0 && zeroes < std::min(a.size(), b.size())) {
    zeroes++;
  }
  a >>= zeroes;
  b >>= zeroes;

  if (b.value_.size() == 1 && b.value_[0] == 0) {
    throw std::runtime_error("division by zero");
  } else if (a.value_.size() < b.value_.size()) {
    return big_integer();
  } else if (b.value_.size() == 1) {
...
big_integer operator>>(big_integer a, int b) {
  ...
  big_integer res;
  res.value_.clear();
  for (size_t i = static_cast<size_t>(b / 32); i < a.value_.size(); ++i) {
...
bool big_integer::is_zero() const {
  return !sign_ && value_.size() == 1 && value_[0] == 0;
}
...
bool big_integer::less_abs(big_integer const &a, big_integer const &b) {
  if (a.value_.size() != b.value_.size()) {
    return a.value_.size() < b.value_.size();
  }
